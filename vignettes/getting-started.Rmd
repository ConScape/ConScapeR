...
# If ConScape is already installed in Julia:
...
library(ConScapeR)
ConScapeR_setup("C:/Users/bram.van.moorter/AppData/Local/Programs/Julia-1.9.3/bin")
```
...
In our example we will consider an independent likelihood of movement (i) and habitat suitability (iii and iv), we assume the cost of movement a function of the likelihood ($c = -log(a)$) and we assume that suitability as a source and target are identical (sources = targets). Define a ConScape Grid:
...
thetas = c(2.5, 1.0, 0.5, 0.1, 0.01, 0.001)
